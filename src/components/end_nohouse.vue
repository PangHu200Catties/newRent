      ...
      <el-table-column label="操作">
        <template slot-scope="scope">
          <el-button
            ...
            type="primary"
            icon="el-icon-edit"
            @click="fun3(scope.row.uid)">房东信息</el-button>
          <el-button
            size="mini"
            type="primary"
            icon="el-icon-edit"
            @click="fun4(scope.row.hid)">房屋图片</el-button>
          <el-button
            size="mini"
            type="danger"
            icon="el-icon-delete"
            @click="fun2(scope.row.hid)">禁用使用</el-button>
        </template>
      </el-table-column>
    </el-table>
...
//        var id=this.$route.params.id
      ...
      fun3:function (uid) {
        this.$router.push({name:'end_landlorduser',params:{uid:uid}})
      },
      fun4:function (hid) {
        this.$router.push({name:'end_housepic',params:{hid:hid}})
      },
      findBy:function () {
        var url='api/endsearchhouse/'+this.search+"/"+this.status+"/"+this.currentPage+"/"+this.pagesize;
        axios.get(url).then(res=>{
          this.houses=res.data.list;
          this.total=res.data.total;
        })
